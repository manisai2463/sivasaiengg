...
export default function TurbineCanvas() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Left Pedestal
      ctx.beginPath();
      ctx.roundRect(cx - shaftLength / 2.2, cy - 30, 40, 60, 4);
      ctx.fill(); ctx.stroke();
      // Right Pedestal
      ctx.beginPath();
      ctx.roundRect(cx + shaftLength / 2.5, cy - 30, 40, 60, 4);
      ctx.fill(); ctx.stroke();

      // Multi-stage blades drawing (HP, IP, LP stages)
      const stages = [
        { offset: -180, scale: 30, count: 12, label: 'HP Stage' },
        { offset: -80, scale: 40, count: 10, label: 'IP Stage' },
        { offset: 40, scale: 55, count: 8, label: 'LP Stage 1' },
        { offset: 140, scale: 70, count: 8, label: 'LP Stage 2' },
      ];
      ...
      stages.forEach((stage) => {
        ...
        for (let i = 0; i < stage.count; i++) {
          ...
          ctx.lineWidth = 4 + (stage.scale / 15);
          ctx.beginPath();
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        stages.forEach(stage => {
          if (Math.abs(p.x - (cx + stage.offset)) < 15) {
      ...
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.roundRect(cx - shaftLength / 2.1, cy - 85, shaftLength * 0.9, 170, 12);
      ctx.stroke();
