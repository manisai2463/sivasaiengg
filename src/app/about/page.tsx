      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20">
          <motion.div
            ...
            className="inline-flex items-center gap-2 bg-slate-200 dark:bg-[#111827]/70 border border-slate-300 dark:border-white/10 px-3 py-1 rounded-full mb-4"
          >
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mb-24">
          ...
          >
            ...
            <p className="text-sm sm:text-base text-black dark:text-gray-400 leading-relaxed whitespace-pre-line">
              {BRIEF_ABOUT}
            </p>

            <div className="grid grid-cols-2 gap-4 mt-6">
              {CORE_VALUES.map((val) => (
                <div key={val.title} className="glass-panel p-5 rounded-xl flex flex-col gap-2 border border-gray-200 dark:border-white/5">
          ...
          >
            {EXPERT_TEAM.slice(0, 2).map((member, index) => (
              <div key={member.name} className="glass-panel p-6 md:p-8 rounded-3xl border border-gray-200 dark:border-white/5 relative overflow-hidden w-full flex flex-col justify-between">
                <div className="absolute top-0 right-0 bg-primary/20 border-b border-l border-gray-200 dark:border-white/5 px-3 py-1.5 text-[10px] text-accent font-mono uppercase tracking-wider rounded-bl-xl">
                ...
                <div className="flex flex-col gap-5">
                  ...
                  <div>
                    <h3 className="text-gray-900 dark:text-white font-bold text-xl">{member.name}</h3>
                    <span className="text-xs text-accent font-mono font-medium block mt-1">{member.role}</span>
                  </div>
                  <p className="text-xs sm:text-sm text-black dark:text-gray-400 leading-relaxed border-t border-gray-200 dark:border-white/5 pt-4">
                    {member.details}
                  </p>
                </div>
              </div>
            ...
            <div className="glass-panel p-6 md:p-8 rounded-3xl border border-gray-200 dark:border-white/5 text-[10px] sm:text-xs text-black dark:text-gray-500 font-mono leading-relaxed">
        ...
        {/* Advisory Team */}
        <section className="border-t border-gray-200 dark:border-white/5 pt-20 mb-24">
          <div className="text-center mb-16">
            <span className="text-xs text-secondary font-mono tracking-widest uppercase block mb-1">
              Advisory Counsel
            </span>
            <h2 className="text-gray-900 dark:text-white font-bold text-2xl md:text-3xl flex items-center justify-center gap-2">
              <Users className="w-6 h-6 text-accent" />
              Expert Power Engineers
            </h2>
            <p className="text-xs sm:text-sm text-black dark:text-gray-500 max-w-lg mx-auto leading-relaxed mt-2">
          ...
          <div className="flex overflow-x-auto snap-x snap-mandatory scroll-smooth md:grid md:grid-cols-3 gap-5 pb-4 md:pb-0 scrollbar-none [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden w-full -mx-4 px-4 md:mx-0 md:px-0">
            {EXPERT_TEAM.slice(2).map((member) => (
              <div key={member.name} className="glass-panel glass-panel-hover p-5 rounded-2xl border border-gray-200 dark:border-white/5 flex flex-col justify-between w-[270px] min-w-[270px] snap-start shrink-0 md:w-auto md:min-w-0 md:shrink">
                <div className="flex flex-col gap-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0">
                    ...
                    <span className="text-[10px] text-black dark:text-gray-500 font-mono font-semibold">{member.experience}</span>
                  </div>
                  <h4 className="text-gray-900 dark:text-white font-semibold text-base mt-2">{member.name}</h4>
                  <span className="text-xs text-secondary font-mono font-medium">{member.role}</span>
                </div>
                <p className="text-xs text-black dark:text-gray-400 leading-relaxed border-t border-gray-200 dark:border-white/5 pt-4 mt-4">
                  {member.details}
                </p>
              </div>
            ))}
        ...
        <section className="border-t border-gray-200 dark:border-white/5 pt-20">
          <div className="text-center mb-16">
            ...
            <h2 className="text-gray-900 dark:text-white font-bold text-3xl">Corporate Timeline</h2>
