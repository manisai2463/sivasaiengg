      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          {SERVICES_LIST.map((service, idx) => (
            ...
            >
              <div>
                ...
                <h3 className="text-gray-900 dark:text-white font-bold text-base md:text-lg mb-1.5 md:mb-3">{service.title}</h3>
                <p className="text-[11px] md:text-xs text-black dark:text-gray-400 mb-4 md:mb-6 leading-relaxed">{service.description}</p>
              </div>

              <ul className="space-y-2 md:space-y-2.5 border-t border-gray-200 dark:border-white/5 pt-3.5 md:pt-5 text-[11px] md:text-xs text-black dark:text-gray-500 leading-normal">
        ...
        <section className="border-t border-gray-200 dark:border-white/5 pt-20">
          <div className="text-center mb-16">
            ...
            <h2 className="text-gray-900 dark:text-white font-bold text-3xl flex items-center justify-center gap-2">
              ...
              >
                <div className="flex flex-col gap-2 md:gap-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-gray-900 dark:text-white font-bold text-sm md:text-base tracking-wide">{ind.name}</h3>
                    <Factory className="w-4 h-4 text-accent" />
                  </div>
                  <p className="text-[11px] md:text-xs text-black dark:text-gray-400 leading-relaxed">
                    {ind.description}
                  </p>
                </div>
                
                <div className="border-t border-gray-200 dark:border-white/5 pt-3 md:pt-4 mt-4 md:mt-6 flex flex-wrap gap-1.5 items-center">
                  <span className="text-[9px] text-black dark:text-gray-500 font-mono uppercase tracking-wider mr-1">
                    Clients Served:
                  </span>
                  {ind.clients.map((client) => (
                    <span key={client} className="px-1.5 py-0.5 rounded bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/5 text-[9px] text-black dark:text-gray-300 font-mono">
                      {client}
                    </span>
                  ))}
                </div>
